...
@router.post("/logout")
async def logout(current_user_id: int = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    return await AuthenticationRepository.logout(current_user_id,db)